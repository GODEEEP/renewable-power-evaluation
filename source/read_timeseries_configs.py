...
def read_godeeep_data(datadir):
  ...
  for res, gd_cf in zip(['solar', 'wind'], [gd_s, gd_w]):
    ...
    if (len(missing_plants) > 0) and (np.nan not in missing_plants):
      ...
      droplist = eia860_y[(eia860_y['resource'] == res) &
                          (eia860_y['plant_code_unique'].isin([i[0] for i in missing_plants]))][
...
def read_eia_923_930(datadir):
  ...
  for yr in range(2007, 2021):
    ...
    eia923_df = pd.read_excel(datadir+r'/EIA923/f923_'+str(yr)+'/'+filename,
                              skiprows=skipr, sheet_name='Page 1 Generation and Fuel Data')
    ...
    if 'netgen_jan' in eia923_df.columns:
      eia923_df = eia923_df.rename(
          columns={'netgen_jan': 'netgen january', 'netgen_feb': 'netgen february',
                   'netgen_mar': 'netgen march', 'netgen_apr': 'netgen april',
                   'netgen_may': 'netgen may', 'netgen_jun': 'netgen june',
                   'netgen_jul': 'netgen july', 'netgen_aug': 'netgen august',
                   'netgen_sep': 'netgen september', 'netgen_oct': 'netgen october',
...
def get_bpa(datadir):
  ...
  for f in bafiles:
    ...
    if y in [7, 8, 9, 10]:
      ...
      ba_yr.columns = ['datetime', 'BPA-Wind']
  ...
  fivemin_idx = pd.date_range(start='2007-01-01 08:00:00', end='2021-01-01 07:55:00',
                              freq='5T', tz='UTC').values.astype('datetime64[s]')
...
def get_nyiso(datadir):
  ...
  # revert EDT to EST
  ba_scada.loc[ba_scada['Time Zone'] == 'EDT', ['datetime']
               ] = ba_scada.loc[ba_scada['Time Zone'] == 'EDT', ['datetime']] - timedelta(hours=1)
  # take hourly average
  ba_scada = ba_scada.set_index('datetime')
  ba_scada = ba_scada.resample('H').mean(numeric_only=True)
...
def get_ercot(resource, datadir):
  ...
  if resource == 'wind':
    idx = pd.date_range(start='2007-01-01 06:15:00', periods=490944,
                        freq='15T', tz='UTC').values.astype('datetime64[s]')
  elif resource == 'solar':
    idx = pd.date_range(start='2011-07-01 05:15:00', periods=333316,
                        freq='15T', tz='UTC').values.astype('datetime64[s]')
  ba_scada = ba_scada.sort_values(by=['Date', 'HE', 'int']).set_index(idx)
  ba_scada['value'] = ba_scada['value'].astype(float)
  ba_scada = pd.DataFrame(ba_scada.rename(columns={'value': resource.capitalize()})[
                          resource.capitalize()]).resample('H').sum()
  ba_scada.index = ba_scada.index.tz_localize('UTC')
  return pd.DataFrame(ba_scada[resource.capitalize()])


def get_caiso(resource, datadir, curt=False):
  fi_list = ['ProductionAndCurtailmentsData-May1_2014-May31_2017.xlsx',
             'ProductionAndCurtailmentsData-Jun1_2017-Dec31_2017.xlsx', 'ProductionAndCurtailmentsData_2018.xlsx',
             'ProductionAndCurtailmentsData_2019.xlsx', 'ProductionAndCurtailmentsData_2020.xlsx']
  ba_scada = pd.DataFrame()
  ...
  if curt == True:
    ...
    ba_scada = ba_scada.merge(
        ba_curt, how='left', left_on=['Date', 'Hour', 'Interval'],
        right_on=['Date', 'Hour', 'Interval'])
    ba_scada = ba_scada.fillna(0)
  # ba_scada['datetime'] = ba_scada['Date']
  fivemin_idx = pd.date_range(start='2014-05-01 07:00:00', periods=701868,
                              freq='5T', tz='UTC').values.astype('datetime64[s]')
  ba_scada['datetime'] = fivemin_idx
  ba_scada = ba_scada.set_index('datetime')
  ba_scada = ba_scada.resample('H').mean(numeric_only=True)
...
def get_isone(resource, datadir):
  if resource == 'wind':
    ...
    for fi in fi_list:
      ...
      isne['datetime'] = pd.to_datetime(isne['Date'].astype(
          str)+' '+isne['Hour'].astype(str)+':'+np.zeros(len(isne)).astype(str))
    ...
    for fi in fi_list:
      ...
      ba_scada = pd.concat([ba_scada, isne])
  est_hour_idx = pd.date_range(start='2011-01-01 05:00:00', end='2021-01-01 04:55:00',
                               freq='H', tz='UTC').values.astype('datetime64[s]')
  ba_scada['datetime'] = est_hour_idx
  ba_scada = ba_scada.set_index('datetime')
  ba_scada.index = ba_scada.index.tz_localize('UTC')
