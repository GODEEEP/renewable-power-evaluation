...
def create_plantlevel_generation(gd_df, eia860m_long, res, monthyears, solar_configs, wind_configs, datadir):
  ...
  for y, m in monthyears:
    ...
    gd_r.loc[(gd_r.index.year == y) & (gd_r.index.month == m), gd_r_cols] = pm.groupby(
        'plant_gen_id').apply(cf_to_gen, pd.DataFrame(gd_df_m), res, solar_configs, wind_configs).T
...
def create_balevel_generation(
        all_plant_gen, res, balist, gd_df, eia930_df, eia923_df, eia860m, subset_list, monthyears, datadir, save_dicts):
  BA_hr_diff = {}
  for thisBA in balist:
    ...
    for y, m in monthyears:
      pm = plants_monthly[['balancing authority code',
                           'nameplate capacity (mw)', 'resource', 'month', 'year', 'plant_gen_id']].drop_duplicates()
    ...
    if thisBA == 'GRIS':
      fixedBA = 'GRID'
      BA_hr = pd.DataFrame(
          {'GODEEEP-' + res: BA.sum(axis=1)}).merge(
          eia930_df.loc[eia930_df['eia_ba_name'] == fixedBA]['value'],
          how='left', left_index=True, right_index=True)
    else:
      BA_hr = pd.DataFrame(
          {'GODEEEP-' + res: BA.sum(axis=1)}).merge(
          eia930_df.loc[eia930_df['eia_ba_name'] == thisBA]['value'],
          how='left', left_index=True, right_index=True)
    ...
    for yr in [*set([i[0] for i in monthyears])]:
      ...
      for m in months:
        ...
        BA_m.loc[(BA_m.index.year == int(yr)) & (BA_m.index.month == m), 'eia923'] = eia923_ba.loc[(
            eia923_ba['year'] == yr) & (eia923_ba['plant id'].isin(pi_thismonth)), month_col].sum().values
    # BA_m = BA_m.iloc[:-1]
