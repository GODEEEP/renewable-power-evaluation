...
def plot_infrastructure_over_time(
        wlist, slist, eia860m_long, all_power_hourly, outdir, plot_capnormdiff=False, savef=False):
  ...
  for this_list, res in zip([wlist, slist], ['wind', 'solar']):
    for this_ba in this_list:
      ...
      ba_hourly_power = ba_hourly_power.groupby(
          [ba_hourly_power.index.year, ba_hourly_power.index.month]).max().reset_index()
      ...
      ba_hourly_power = ba_hourly_power[ba_hourly_power['year-mo']
                                        .isin(ba_list['year-mo'].tolist())].reset_index(drop=True)
      ba_hourly_power['capacity'] = tmp['nameplate capacity (mw)']
      ba_hourly_power['capacity percent diff'] = (
          ba_hourly_power['scada']-ba_hourly_power['GODEEEP-'+res])/ba_hourly_power['capacity']
      ba_hourly_power['percent diff'] = (
          ba_hourly_power['scada']-ba_hourly_power['GODEEEP-'+res])/ba_hourly_power['scada']
      ...
      plt.plot(tmp['year-mo'], tmp['nameplate capacity (mw)'],
               label='godeeep-capacity', color='tab:blue', linestyle='--')
      plt.plot(ba_list['year-mo'], ba_list['nameplate capacity (mw)'],
               label=this_ba+'-capacity', color='k', linestyle='--')
      plt.plot(ba_hourly_power['year-mo'], ba_hourly_power['GODEEEP-'+res],
               label='godeeep-power', color='tab:blue', linestyle='-')
      ...
      if plot_capnormdiff:
        ...
        minorx_ticks = pd.RangeIndex(0, len(tmp), 6)
        plt.plot(
            ba_hourly_power['year-mo'],
            ba_hourly_power['capacity percent diff'],
            color='tab:blue', linestyle='-', alpha=0.9)
        ax.set_xticks(majorx_ticks)
        ax.set_xticks(minorx_ticks, minor=True)
        plt.xticks(rotation=45)  # Rotates X-Axis Ticks by 45-degrees
        ax.grid(which='minor', alpha=0.2)
...
def setup_timeseries(df, compute_cf, capacity, cols):
  ...
  if compute_cf:
    ...
    df = df.merge(
        capacity, how='left', left_on=[df.index.year, df.index.month],
        right_on=['year', 'month']).set_index(
        df.index)
...
def plot_whiskers(dd, freq, res, comp, plotdir, thesecols, region, agg, ymax, savef, showf):
  ...
  dd = pd.melt(
      dd.reset_index(),
      id_vars=[grouping],
      value_vars=['GODEEEP-' + res, comp[0]],
      var_name='dataset', value_name='Capacity Factor')
...
def plot_grouped_box(
        all_power_hourly, eia860m_long, freq, res, balist, outdir, compute_cf, comp, mask, aggregation, season_disagg,
        savef=False, showf=False):
  tabledir = outdir+r'/tables'
  if not os.path.exists(tabledir):
    os.makedirs(tabledir)
  if savef:
  ...
  if aggregation == 'by BA':
    ...
    for BA in balist:
      ...
      capacity = eia860m_long[(eia860m_long['balancing authority code'] == BA) &
                              (eia860m_long['resource'] == res)].groupby(
          ['year', 'month'])['nameplate capacity (mw)'].sum().reset_index()
      capacity['year-mo'] = capacity[['year', 'month']].astype(str).agg('-'.join, axis=1)
      dd = dd[(dd.index.year > 2006) & (dd.index.year < 2021)]
      dd = dd.merge(
          capacity, how='left', left_on=[dd.index.year, dd.index.month],
          right_on=['year', 'month']).set_index(
          dd.index)
    ...
    for bas_in_nerc, nerc_name in zip([wecc, eic, erco], nerc_zones):
      ...
      nerc_wide = pd.DataFrame(columns=['GODEEEP-'+res]+comp+['nameplate capacity (mw)'], index=pd.date_range(
          start='2007-01-01', end='2021-01-01', inclusive='left', freq='1H', tz='UTC').values.astype('datetime64[s]'))
...
def plot_grouped_bars(
        all_power_hourly, eia860m_long, freq, res, balist, outdir, compute_cf, comparison_datasets, mask, agg, s_agg,
        savef=False, showf=False):
  tabledir = outdir+r'/tables'
  if not os.path.exists(tabledir):
    os.makedirs(tabledir)
...
def plot_correlation(
        all_power_hourly, eia860m_long, freq, res, these_bas, outdir, compute_cf, comparison_datasets, mask_bad_data,
        agg, s_agg, savef=False, showf=False):
  tabledir = outdir+r'/tables'
  if not os.path.exists(tabledir):
    os.makedirs(tabledir)
  if savef:
  ...
  if agg == 'by BA':
    ...
    a_n = 'byregion'
    stats_df = pd.DataFrame(columns=pd.MultiIndex.from_product(
        [nerc_zones, ['GODEEEP-'+res]+comparison_datasets]), index=list(range(2007, 2021)))
    ...
    for bas_in_nerc, nerc_name in zip([wecc, eic, erco], nerc_zones):
      ...
      for BA, ba_idx in zip(plot_bas, range(len(plot_bas))):
        ...
        if mask_bad_data == True:
          ...
          low_cf_mo_idx, low_cf_yr_idx = monthly_cf.loc[(monthly_cf == 0).any(axis=1)].index.get_level_values(
              1), monthly_cf.loc[(monthly_cf == 0).any(axis=1)].index.get_level_values(0)
...
# we want a similarity metric where if we separate these histograms with more zeros,
# it tells us that they are less similar, and if we remove zeros or make them overlap,
# it tells us that they are more similar.
...
def get_emd_scada(all_power_hourly, eia860m_long, balist, res, dates, comp, tabledir, normalize=False, robust=True):
  ...
  for bb in balist:
    ...
    df_emd = dd.loc[(dd.index.year.isin(yr)) & (dd.index.month.isin(mo))
                    & (dd.index.day.isin(da)), ['GODEEEP-'+res, 'scada']]
    ...
    if normalize:
      capacity = eia860m_long[(eia860m_long['balancing authority code'] == bb) &
                              (eia860m_long['resource'] == res)].groupby(
          ['year', 'month'])['nameplate capacity (mw)'].sum().reset_index()
      capacity['year-mo'] = capacity[['year', 'month']].astype(str).agg('-'.join, axis=1)
...
def plot_emd(
    all_power_hourly, eia860m_long, dates, res, these_bas, outdir, compute_cf, comparison_datasets, mask, agg,
        s_agg, savef=False, showf=False):
  tabledir = outdir+r'/tables'
  if not os.path.exists(tabledir):
    os.makedirs(tabledir)
  if savef:
    ...
    if not os.path.exists(plotdir):
      os.makedirs(plotdir)
  emd = dict(zip(these_bas, get_emd_scada(all_power_hourly, eia860m_long, these_bas, res,
             dates, comparison_datasets[0], tabledir, normalize=compute_cf, robust=True)))
  # return get_emd_scada(all_power_hourly,eia860m_long,these_bas,res,dates,
  # comparison_datasets[0],normalize=compute_cf,robust=True)
